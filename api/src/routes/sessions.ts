...
sessions.post('/', async (c) => {
  ...
  const { list_id, votes_per_round, submitter_name } = parsed.data;
...
sessions.get('/:room_code', async (c) => {
  ...
  return c.json({ session, participants, games, voted_count: voteStats.voted_count });
});
...
sessions.post('/:room_code/start', async (c) => {
  ...
  const [updated] = await sql`UPDATE sessions SET status = 'voting' WHERE id = ${session.id} RETURNING *`;
  return c.json(updated);
});

sessions.post('/:room_code/next-round', async (c) => {
  const parsed = nextRoundBody.safeParse(await c.req.json());
  if (!parsed.success) return errorResponse(c, 400, parsed.error.issues[0].message);
  const { participant_id, tied_game_ids } = parsed.data;
  ...
  const [updated] = await sql`
    UPDATE sessions SET current_round = ${next_round}, status = 'voting'
...
sessions.post('/:room_code/complete', async (c) => {
  const room_code = c.req.param('room_code').toUpperCase();
  const [updated] = await sql`
    UPDATE sessions SET status = 'complete' WHERE room_code = ${room_code} RETURNING *
  `;
